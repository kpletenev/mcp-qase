...
export const GetResultsByStatusSchema = z.object({
  ...
  status: z.string().describe('Status to filter by (e.g., failed, passed, skipped, blocked, invalid)'),
  unique: z.boolean().optional().describe('Return only unique test cases (removes duplicates from retries/reruns)'),
  limit: z.string().optional().describe('Maximum number of results to return'),
  offset: z.string().optional().describe('Number of results to skip for pagination'),
  from: z.string().optional().describe('Filter results from this timestamp (format: YYYY-MM-DD HH:mm:ss)'),
  to: z.string().optional().describe('Filter results to this timestamp (format: YYYY-MM-DD HH:mm:ss)'),
});
